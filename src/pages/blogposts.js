...
const mockdata = [
  {
    ...
    description:
      "Cryptography is fundamental for securing online communications. Students delve into encryption algorithms to understand how data is protected.",
  ...
  },
  {
    ...
    description:
      "Web development combines creativity with technical skills. It's a gateway for students to bring innovative ideas to life online.",
  ...
  },
  {
    ...
    description:
      "Artificial Intelligence (AI) is reshaping industries worldwide. It offers students a glimpse into the future of technology and its applications.",
  ...
  },
  {
    ...
    description:
      "Quantum computing represents the next frontier in computing technology, offering unprecedented processing power. Its potential to solve complex problems faster than classical computers sparks interest among tech enthusiasts.",
  ...
  },
  {
    ...
    description:
      "As AI technologies become more integrated into our daily lives, ethical considerations are increasingly important. Concerns about privacy, bias, and autonomy are central to the debate.",
  ...
  },
  {
    ...
    description:
      "Blockchain technology is transforming data security with its decentralized and transparent nature. It offers a new way to secure and verify transactions without the need for central authorities.",
  ...
  },
  {
    ...
    description:
      "Edge computing is set to revolutionize how data is processed, bringing computation closer to data sources. This approach reduces latency and increases efficiency.",
  ...
  },
  {
    ...
    description:
      "Cybersecurity is an ever-evolving field, with new threats and challenges emerging regularly. Staying ahead of these threats is critical for protecting sensitive information.",
...
const useBlogs = (page, postsPerPage, searchQuery) => {
  const filteredBlogs = useMemo(() => {
    if (!searchQuery) return mockdata;
    return mockdata.filter((blog) => blog.title.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [searchQuery]);

  return useMemo(() => {
    return applyPagination(filteredBlogs, page, postsPerPage);
  }, [filteredBlogs, page, postsPerPage]);
};
...
const Page = () => {
  ...
  const blogs = useBlogs(page, postsPerPage, searchQuery);

  const pageCount = Math.ceil(blogs.length / postsPerPage);
  const startIndex = (page - 1) * postsPerPage;
  const visiblePosts = blogs.slice(startIndex, startIndex + postsPerPage);
        ...
        <Container maxWidth="xl">
          <Stack spacing={3}>
            ...
            <Box sx={{ display: "flex", justifyContent: "center" }}>
              <Pagination
                count={Math.ceil(mockdata.length / 6)} // Calculate the total number of pages based on the total number of blog posts
                size="small"
                page={page}
      ...
      >
        <Box sx={{ ...modalStyle, width: "80%", margin: "auto" }}>
          {currentBlogPost ? (
            ...
            <Typography variant="body1">No blog post selected.</Typography>
          )}
          <Box sx={{ mt: 2, display: "flex", justifyContent: "flex-end" }}>
